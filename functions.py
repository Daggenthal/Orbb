	...
	while True:
		...
		if response == '1':
			...
			if response == '1':
				...
				run(['cd /tmp/ && sudo rsync ServerBackup.tar.gz', userName + '@' + ipAddress + ':/tmp/'], shell=True, check=True)
...
def serverSetup():
	while True:
		...
		if response == '1':
			...
			if 'debian' in OS:
				run(['sudo apt install -y nginx mariadb certbot postfix python3-certbot-nginx'], shell=True, check=True)
			elif 'ubuntu' in OS:
				run(['sudo apt install -y nginx mariadb certbot postfix python3-certbox-nginx'], shell=True, check=True)
			elif 'fedora' in OS:
				run(['sudo dnf install -y epel-release && sudo dnf install -y nginx mariadb certbot postfix python3-certbot-nginx'], shell=True, check=True)
			elif 'arch' in OS:
				run(['sudo pacman -S --noconfirm nginx mariadb certbot postfix python3-certbot-nginx'], shell=True, check=True)
			elif 'opensuse' in OS:
				run(['sudo zypper install -y nginx mariadb certbot postfix python3-certbot-nginx'], shell=True, check=True)
			elif 'freebsd' in OS:
				run(['sudo pkg install -y nginx mariadb certbot postfix python3-certbot-nginx'], shell=True, check=True)
